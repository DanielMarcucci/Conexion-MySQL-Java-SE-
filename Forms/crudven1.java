...
import Estructuras.ventas;
import Estructuras.ventasdet;
 ...
 */
public class crudven1 extends javax.swing.JFrame {
    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        ...
        iqr = "INSERT INTO ventas(noFacPadre, nitCliHijo, fecha, total) VALUES(?,?,?,?)";
        ivl = new Object[]{noFac,nit,fecha,txtTotal.getText()};
        uqr = "UPDATE ventas SET nitCliHijo=?, fecha=?, total=? WHERE noFacPadre=?";
        uvl = new Object[]{nit,fecha,total,noFac};
        ejSQL.insertUpdate(iqr, ivl, uqr, uvl);
        
        if(!nuevaFac){
            uqr = "DELETE FROM ventasdet WHERE noFacHijo=?";
            uvl = new Object[]{txtNoFac.getText()};
            ejSQL.delete(uqr,uvl);
        }
        iqr  = "INSERT INTO ventasdet(noFacHijo, idArtHijo, cant, preUnit, monto) VALUES(?,?,?,?,?)";
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        ...
        if(YES_NO_OPTION == showConfirmDialog(null,"¿Está seguro de querer eliminar este registro?","Eliminar Registro",YES_NO_OPTION,QUESTION_MESSAGE)){
            qr = "DELETE FROM ventasdet WHERE noFacHijo=?";
            vl = new Object[]{txtNoFac.getText()};
            ejSQL.delete(qr,vl);
            
            qr = "DELETE FROM ventas WHERE noFacPadre=?";
            vl = new Object[]{txtNoFac.getText()};
            ejSQL.delete(qr,vl);
            limpiarBtn(true);
        }
    ...
    private void btnBusProActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBusProActionPerformed
        ...
        listaCli = bCli(tblBusPro, txtBusPro.getText(), rd);
    ...
    private void bCliSele(JTable tabla) {
        try {
            ...
            if(!listaCli.get(row).isEstado()){
                showMessageDialog(null,"No se puede usar este cliente porque no está activo", "Estado Inactivo", JOptionPane.WARNING_MESSAGE, null);
                txtNit.requestFocus();
                return;
            }
    ...
    public void rFac(){
        ...
        String qr;
        Object[] vl;
        qr = "SELECT * FROM ventas v INNER JOIN clientes c ON c.nitCliPadre=v.nitCliHijo WHERE noFacPadre=?";
        vl = new Object[]{txtNoFac.getText()};
        BeanListHandler blh = new BeanListHandler(ventas.class);
        List<ventas> lVen = ejSQL.select(qr, blh, vl);
        ...
        qr = "SELECT * FROM ventasdet cd INNER JOIN articulos a ON cd.idArtHijo=a.idArtPadre WHERE noFacHijo=?";
        blh = new BeanListHandler(ventasdet.class);
        List<ventasdet> lVenDet = ejSQL.select(qr, blh, vl);
        
        mp.tblVaciar(tblDet);
        String[] columnas = new String[5];
        DefaultTableModel filas = (DefaultTableModel) tblDet.getModel();
        lVenDet.forEach(item->{
    ...
    public void rCli(){
        ...
        BeanListHandler blh = new BeanListHandler(clientes.class);
        List<clientes> lCli = ejSQL.select(qr, blh, vl);
        
        if(lCli.isEmpty()){
            txtNit.setText("");
            showMessageDialog(null,"Este cliente no existe", "No Existe", JOptionPane.WARNING_MESSAGE, null);
            txtNit.requestFocus();
            return;
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(crudven1.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
